...
def upd_usrscre(user, score):##
    try:
        ...
        Uts = """ INSERT INTO users_score (username, score) VALUES (%s,%s) """
        Ut = """ INSERT INTO users (username) VALUES (%s) """
        cursor.execute(Uts,(user, score))
        ...
        if over == 0: #if game is not over
            ...
            if tim == 1:
                #for i in scorel:
                    #if i not in lscorel:
                        #lscorel.append(i)
                upd_usrscre(user, score)
                """with open("C:\\Users\\User\\Desktop\\Программирование\\my\\pp2-22B030391\\tsis10\\Second\\score.txt","a") as f: #"w+") as f:
                    f.write("S")
                    f.write("\n")

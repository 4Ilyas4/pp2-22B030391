...
def main():
    ...
    while running:
        ...
        if over == 0: #
            ...
            if snake.check_collision(tox):  
                pygame.mixer.Sound.play(eatf)
                if len(snake.body) == 1:
                    over = 1
                score -= 1  
                if len(snake.body)>1:
                ...
                tox.location.x = random.randint(0, WIDTH // BLOCK_SIZE - 1)   
                tox.location.y = random.randint(0, HEIGHT // BLOCK_SIZE - 1) 
